...
<script>
	import { getUserById, saveUser } from "@/service/User";
	...
	export default {
		...
		methods: {
			checkUserId() {
				...
				// 		})
				// 	)
					getUserById(this.user.userId).then((response) => {
			...
			register() {
				...
				if (this.user.userId.toString().length != 11){
				        alert("手机号码必须是11位数字！");
				        return;
				      }
				...
				// 	.post("UserController/saveUser", this.$qs.stringify(this.user))
					...
					saveUser(this.user.userId,this.user.password,this.user.userName,this.user.userSex).then((response) => {
